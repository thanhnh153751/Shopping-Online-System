...
public class HistoryOrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String cid_r = request.getParameter("id");
        if(a==null) response.sendRedirect("login");
        else{
            if(cid_r== null){
            int cid = a.getCustomerID();
            List<Order> listOrder = d.getOrderHistoryCustomer(cid);
            request.setAttribute("listOd", listOrder);
            request.getRequestDispatcher("myorder.jsp").forward(request, response);
                
            }else{
            int cid = Integer.parseInt(cid_r);
            List<Order> listOrder = d.getOrderHistoryCustomer(cid);
            request.setAttribute("listOd", listOrder);
            request.getRequestDispatcher("myorder.jsp").forward(request, response); 
            }
